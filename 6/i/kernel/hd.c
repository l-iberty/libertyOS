...
void Task_hd()
{
	...
	for (;;)
	{
		...
		switch (msg.value)
		{
			...
			case DEV_READ:
			{
				hd_read(msg.DRIVE, msg.SECTOR, msg.BUF, msg.LEN);
				break;
			}
			case DEV_WRITE:
			{
				hd_write(msg.DRIVE, msg.SECTOR, msg.BUF, msg.LEN);
				break;
			}
			default:
			{
...
void hd_cmd_out(HD_CMD* cmd)
{
	...
	while ((in_byte(REG_STATUS) & 0x80)) {_printf("-");}
