...
STATIC int alloc_page(int nr_pages, uint32_t *pte, uint32_t *pde_idx, uint32_t *pte_idx);
STATIC struct page_list *alloc_frame(int nr_pages);
STATIC int check_free_page(uint32_t *pte, uint32_t idx, int n);
STATIC int check_free_frame(struct page_list *p, int n);
...
void *do_vm_alloc()
{
	...
	if (vm_addr)
	{
		...
		{
			printf("\n#ERROR#{do_vm_alloc} virual memory not enough.#1");
			return NULL;
		}
	}
	else
	{
		...
		{
			printf("\n#ERROR#{do_vm_alloc} virual memory not enough.#2");
			return NULL;
		}
