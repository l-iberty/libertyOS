...
/*	printf("\n{MM}    RAM Size: %dM", ram_size >> 20);*/
	 ...
	 * "0号页表"), 并将其拷贝 (NR_NATIVE_PROCS + NR_TASKS - 1) 份, 用作其余进程的页表.
	 */
	 
	/* 全部进程的二级页表占据的页面数 */
	reserved_pages = (NR_NATIVE_PROCS + NR_TASKS) * PAGE_TABLE_PAGES;
...
/*	printf("\n{MM}    Reserved pages: %d", reserved_pages);*/
	...
	/* 拷贝0号页表 */
	uint32_t p = mi->page_dir_base + PAGE_TABLE_PAGES * PAGE_SIZE;
	for (i = 0; i < NR_NATIVE_PROCS + NR_TASKS - 1; i++)
	{
		memcpy((void*)p, (void*)mi->page_dir_base, PAGE_TABLE_PAGES * PAGE_SIZE);
		p += PAGE_TABLE_PAGES * PAGE_SIZE;
	}
	
	/* 将各个进程的页目录地址记录到 PCB 中 */
	struct proc *p_proc = &FIRST_PROC;
	for (i = 0; i < NR_NATIVE_PROCS + NR_TASKS; i++, p_proc++)
	{
		p_proc->page_dir_base = mi->page_dir_base + i * PAGE_TABLE_PAGES * PAGE_SIZE;
		//printf("\n%.8x", p_proc->page_dir_base);
	}
}
