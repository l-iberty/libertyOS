...
/* 设置PDE和PTE属性位时, 根据访问情况确定 */
				...
				if (pde[pde_idx] == 0)
				{
					pde[pde_idx] = (mi->page_tbl_base + pde_idx * PAGE_SIZE) | pg_attr;
				}
#else
/* 设置页面访问权限为"可读写" */
				p->PROTECT = PAGE_READWRITE;
	...
	else /* CASE-3: 访问权限非法（只考虑读写权限） */
	{
		if (PG_ERR_CODE_WR(err_code) == 1)
		{
			panic("LA = 0x%.8x-The access causing the fault was a write.", get_cr2());
		}
		else
		{
			panic("LA = 0x%.8x-The access causing the fault was a read.", get_cr2());
		}
		__asm__("hlt");
	}
}
