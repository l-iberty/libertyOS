...
struct task task_table[NR_PROCS] = {{Init, task_stack_init},   {TaskA, task_stack_a},    {TaskB, task_stack_b},
                                    {TaskC, task_stack_c},     {TaskD, task_stack_d},    {TaskE, task_stack_e},
                                    {TaskTTY, task_stack_tty}, {TaskHD, task_stack_hd},  {TaskFS, task_stack_fs},
                                    {TaskMM, task_stack_mm},   {TaskEXE, task_stack_exe}};

SYSCALL syscall_table[NR_SYSCALL] = {sys_get_ticks, sys_sendrecv,        sys_getpid,         sys_getppid, sys_printk,
                                     sys_sem_init,  sys_sem_post,        sys_sem_wait,       sys_getcr3,  sys_phymemcpy,
                                     sys_sleep,     sys_get_first_chunk, sys_set_first_chunk};
...
uint32_t sys_getpid() { return p_current_proc->pid; }

uint32_t sys_getppid() { return p_current_proc->pid_parent; }
...
void *sys_get_first_chunk() { return p_current_proc->mm.first_chunk; }

void sys_set_first_chunk(void *addr) { p_current_proc->mm.first_chunk = addr; }
...
void failure(char *exp, char *file, char *base_file, int line) {
  char buf[256] = {0};
  sprintf(buf, "{exp: %s, file: %s, base_file: %s, line: %d}", exp, file, base_file, line);
...
STATIC int empty(struct proc_queue *queue) { return (queue->count == 0); }
