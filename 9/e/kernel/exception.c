...
void put_excep_handler(int vecno, EXCEP_HANDLER handler) { exception_table[vecno] = handler; }

/* #UD */
void do_invalid_opcode(int vecno, uint32_t err_code, uint32_t eip, uint16_t cs, uint32_t eflags) {
  panic("-------#UD-------\nPID: %d\nERR_CODE: 0x%.8x\nCS: 0x%.4x EIP: 0x%.8x EFLAGS: 0x%.8x laddr: 0x%.8x",
        p_current_proc->pid, err_code, cs, eip, eflags, va2la(proc_table + p_current_proc->pid, eip));
...
void do_general_protection(int vecno, uint32_t err_code, uint32_t eip, uint16_t cs, uint32_t eflags) {
  panic("-------#GP-------\nPID: %d\nERR_CODE: 0x%.8x\nCS: 0x%.4x EIP: 0x%.8x EFLAGS: 0x%.8x", p_current_proc->pid,
        err_code, cs, eip, eflags);
