...
void TaskHD() {
  printf("\n---------{TaskHD} Page Dir Base: 0x%.8x---------", getcr3());

  init_hd();

  for (;;) {
    sendrecv(RECEIVE, ANY, &hd_msg);
    switch (hd_msg.value) {
    case DEV_OPEN: {
      do_hd_open();
      break;
...
STATIC void wait_disk() {
  while ((in_byte(REG_STATUS) & ATA_STATUS_BSY))
    ; /* do nothing */
}
...
STATIC void disp_hd_info() {
  ...
  printf("\n{HD}    Capabilities: 0x%.4x (LBA supported: %s)", cap, (cap & 0x0200) ? "YES" : "NO");

  uint16_t cmd_set_supported = hd_buf[83];
  printf("\n{HD}    LBA48 supported: %s", (cmd_set_supported & 0x0400) ? "YES" : "NO");
}
