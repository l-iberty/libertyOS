...
void tty_backspace(struct tty *p_tty) {
  ...
  uint32_t min_cursor = (p_tty->p_console->current_start_addr % SCREEN_WIDTH) + 2; /* 终端提示符有 2 个字符 */
...
/* 将可打印字符放进缓冲区 */
void tty_put_text_ch(struct tty *p_tty, char ch) { p_tty->kb_in.buf_text[p_tty->kb_in.rear++ % KB_BUFSIZE] = ch; }
