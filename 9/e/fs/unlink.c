...
int do_unlink() {
  ...
  if (*(char *)fs_msg.PATHNAME != '/') {
    printf("\n#ERROR#-do_unlink: invalid pathname: %s (pathname should start with root dir \'/\')", fs_msg.PATHNAME);
    return -1;
  }

  memcpy(va2la(proc_table + PID_TASK_FS, filename), va2la(pcaller, fs_msg.PATHNAME + 1), /* `+1` skip root dir `/` */
         namelen);
  filename[namelen] = 0;

  int nr_inode = find_file(filename);
  if (nr_inode == 0) {
    printf("\n#ERROR#-do_unlink: file \"%s\" does not exist {PID:%d}", fs_msg.PATHNAME, pcaller->pid);
    return -1;
  }
  ...
  if (pin >= inode_table + NR_INODES) {
    printf("\n#ERROR#-do_unlink: invalid i-node {PID:%d}", pcaller->pid);
    return -1;
  }

  if (pin->i_mode & I_MODE_DIR) {
    printf("\n#ERROR#-do_unlink: cannot remove a dir {PID:%d}", pcaller->pid);
    return -1;
  }

  if (pin->i_mode & I_MODE_CHARDEV) {
    printf("\n#ERROR#-do_unlink: cannot remove a char-device {PID:%d}", pcaller->pid);
    return -1;
  }

  if (pin->i_mode & I_MODE_NORMAL) {
    if (pin->i_cnt > 0) {
      printf("\n#ERROR#-do_unlink: file \"%s\" is being used by some procs {PID:%d}", fs_msg.PATHNAME, pcaller->pid);
      return -1;
    }
