...
STATIC struct i_node *alloc_inode(int nr_inode, uint32_t mode, uint32_t size, uint32_t start_sector,
                                  uint32_t nr_sector);
...
int do_open() {
  ...
  if (*(char *)fs_msg.PATHNAME != '/') {
    printf("\n#ERROR#-do_open: invalid pathname: %s (pathname should start with root dir \'/\')", fs_msg.PATHNAME);
    return -1;
  }
    ...
    if (pcaller->filp[j]) {
      ...
      if (pin) {
        if (pin->i_nr_inode == nr_inode) {
          printf("\n#ERROR#-do_open: the file \"%s\" has already been opened {PID:%d}", fs_msg.PATHNAME, pcaller->pid);
          return -1;
        }
      }
    }
...
STATIC struct i_node *create_file(char *filename, int flags) {
  ...
  pin = alloc_inode(nr_inode, I_MODE_NORMAL, /* normal file */
                    0,                       /* file size*/
