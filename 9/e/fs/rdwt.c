...
size_t do_rdwt() {
  ...
  int start_sector = pin->i_start_sector + (pfd->fd_pos >> SECTOR_SIZE_SHIFT);
  ...
  if (pfd->fd_pos > pin->i_size) {
    ...
    assert(nr_inode > 0);
    memcpy(inode_buf + INODE_DISK_SIZE * (nr_inode - 1), va2la(pcaller, pcaller->filp[fd]->fd_inode), INODE_DISK_SIZE);
